...
@st.cache_resource
def load_model():
    return spacy.load("E:/CV Parser App/56 F1 Score") 
...
def process_resume(text, filename):
    ...
    extracted_data['raw_text'] = clean_resume  
    return extracted_data
...
if parsed_results:
    st.subheader("Extracted Details (JSON View + Text)")
    ...
    for parsed_data in parsed_results:
        with st.expander(f"Details for {parsed_data['filename']} (Execution Time: {parsed_data['execution_time_sec']} sec)") :
            ...
            st.write(parsed_data['raw_text']) 
            st.markdown("---")
            st.markdown("**Extracted Entities:**")
            st.json({k: v for k, v in parsed_data.items() if k not in ['raw_text']})
    ...
    st.dataframe(df.drop(columns=['raw_text']))  

    csv = df.drop(columns=['raw_text']).to_csv(index = False).encode('utf-8')
    st.download_button(
